...
const levels = [
  { number: "01", title: "Beginner", stage: "Building the foundation" },
  { number: "02", title: "Intermediate", stage: "Deepening technique" },
  { number: "03", title: "Advanced", stage: "Preparing the artist" },
];
    ...
    <main className="classes-page">
      ...
      <section className="classes section">
        ...
        <div className="class-list">
          {levels.map((item) => <article key={item.number}><span className="class-number">{item.number}</span><div><h3>{item.title}</h3></div><span className="level">{item.stage}</span><span className="round-arrow">✦</span></article>)}
        </div>
      </section>
